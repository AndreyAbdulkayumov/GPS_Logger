...
void Display_Write_String(uint16_t x, uint16_t y, char *str,
		uint16_t back_color, uint16_t char_color, uint16_t size)
{
	//Переменная счетчик для вывода следующего сивмола на НОВОМ знакоместе
    uint16_t next = 0;
...
void Display_Write_Number(uint16_t x, uint16_t y, int number,
		uint16_t back_color, uint16_t char_color, uint16_t size)
{
...
void Display_Write_Time(uint16_t x, uint16_t y, uint32_t sec,
		uint16_t back_color, uint16_t char_color, uint16_t size)
{
	//Вспомогательные переменные для хранения разрядов
	uint8_t ed_sec, dec_sec, ed_min, dec_min, ed_hour, dec_hour;
